<script lang="ts">
  ...
  const items: { route: Tab; label: string }[] = [
    ...
    { route: 'altro', label: 'Altro' },
  ];

  const activeTab: Tab = $derived(
    nav.seg === 'poi'
      ? 'citta'
      : nav.seg === 'frasario' || nav.seg === 'spese' || nav.seg === 'emergenze' || nav.seg === 'album'
        ? 'altro'
        : isTab(nav.seg)
          ? nav.seg
          : 'oggi',
  ...
  const icons: Record<Tab, string> = {
    ...
    altro: 'M12 8a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm0 6a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm0-12a2 2 0 1 0 0 4 2 2 0 0 0 0-4Z',
  };
</script>
